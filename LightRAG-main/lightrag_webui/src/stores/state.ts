...
const useBackendStateStoreBase = create<BackendState>()((set, get) => ({
  ...
  check: async () => {
    ...
    if (health.status === 'healthy') {
      ...
      const backendEnableRerank = Boolean(health.configuration?.enable_rerank)
      const currentEnableRerank = Boolean(useSettingsStore.getState().querySettings?.enable_rerank)
      if (!backendEnableRerank && currentEnableRerank) {
        useSettingsStore.getState().updateQuerySettings({ enable_rerank: false })
      }
