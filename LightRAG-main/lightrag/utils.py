...
def parse_provenance_from_chunk(chunk_data: dict, file_path: str) -> dict:
    """从 chunk 数据中解析溯源信息。
    ...
    Returns:
        溯源信息字典
    """
    return create_source_provenance(
        file_path=file_path,
...
def merge_provenances(
    ...
    for prov in existing + new:
        key = (prov.get("doc_id", ""), prov.get("chunk_id", ""))
        if key not in seen:
            seen.add(key)
            merged.append(prov)
