    ...
    if use_azure:
        ...
        if not api_key:
            ...
            if not api_key:
                ...
                is_custom_endpoint = bool(base_url) or (
                    env_base is not None and env_base != "https://api.openai.com/v1"
                )
                if is_custom_endpoint:
                    api_key = "sk-no-key-required"
                    logger.warning(
                        "OPENAI_API_KEY is not set; using a placeholder key for an OpenAI-compatible endpoint."
                    )
                else:
