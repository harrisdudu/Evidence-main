...
# Force common MIME types, especially for Windows
...
def check_frontend_build():
    ...
    if not assets_dir.exists() or not any(assets_dir.glob("*.js")):
        ASCIIColors.yellow("\n" + "=" * 80)
        ASCIIColors.yellow("WARNING: Frontend Build Incomplete")
        ASCIIColors.yellow("=" * 80)
        ASCIIColors.yellow("Found index.html but missing required built assets under api/webui/assets.")
        ASCIIColors.yellow("The API server will start without the WebUI interface.")
        ASCIIColors.yellow("\nTo rebuild WebUI, run:\n")
        ASCIIColors.cyan("    cd lightrag_webui")
        ASCIIColors.cyan("    bun install --frozen-lockfile")
        ASCIIColors.cyan("    bun run build")
        ASCIIColors.cyan("    cd ..")
...
def create_app(args):
    ...
    async def redirect_to_webui():
        ...
        if webui_assets_exist:
            # Always use trailing slash for SPA routing stability
            return RedirectResponse(url="/webui/")
        else:
            return RedirectResponse(url="/docs")

    @app.get("/webui")
    async def redirect_webui_no_slash():
        """Ensure /webui always redirects to /webui/"""
        return RedirectResponse(url="/webui/")
    ...
    class SmartStaticFiles(StaticFiles):  # Renamed from NoCacheStaticFiles
        async def get_response(self, path: str, scope):
            try:
                ...
                # If 404 for a path that doesn't look like an asset, serve index.html (for SPA routing)
                if e.status_code == 404 and "." not in path.split("/")[-1]:
            ...
            is_html = (
                ...
                or normalized_path == ""
                or response.media_type == "text/html"
            )
            ...
            if is_html:
                ...
                "assets/" in normalized_path
            ):  # Assets (JS, CSS, images, fonts) generated by Vite with hash in filename
                response.headers["Cache-Control"] = (
                    "public, max-age=31536000, immutable"
                )

            # Ensure correct Content-Type for critical assets
