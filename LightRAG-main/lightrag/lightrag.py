...
# 导入标准库
import traceback  # 用于获取异常的堆栈跟踪信息
import asyncio  # 异步编程支持
import configparser  # 配置文件解析
import inspect  # 检查对象属性和源代码
import os  # 操作系统接口
import time  # 时间处理
import warnings  # 警告控制

# 导入dataclasses相关工具，用于创建数据类
from dataclasses import asdict, dataclass, field, replace
from datetime import datetime, timezone  # 日期时间处理
from functools import partial  # 函数偏函数创建
...
# 导入LightRAG内部模块
from lightrag.prompt import PROMPTS  # 提示词模板
from lightrag.exceptions import PipelineCancelledException  # 管道取消异常
...
# 加载当前目录下的.env文件，允许每个LightRAG实例使用不同的.env文件
# 操作系统环境变量优先级高于.env文件
...
class LightRAG:
    ...
    def __post_init__(self):
        """LightRAG数据类初始化后的处理方法.

        该方法在数据类初始化完成后自动调用，用于:
        1. 初始化共享数据
        2. 创建工作目录
        3. 验证存储实现兼容性
        4. 初始化分词器
        ...
        # 确保vector_db_storage_cls_kwargs包含必需字段
        ...
        # 初始化嵌入函数
        # 步骤1: 在应用rate_limit装饰器之前捕获embedding_func和max_token_size
        ...
        # 恢复原始EmbeddingFunc对象（asdict会将其转换为dict）
        global_config["embedding_func"] = original_embedding_func

        _print_config = ",\n  ".join([f"{k} = {v}" for k, v in global_config.items()])
        logger.debug(f"LightRAG init with param:\n  {_print_config}\n")

        # 步骤2: 应用优先级包装装饰器到EmbeddingFunc的内部函数
        # 创建一个新的EmbeddingFunc实例来包装函数，避免修改调用者的对象
        ...
        self.relationships_vdb: BaseVectorStorage = self.vector_db_storage_cls(  # type: ignore
            ...
            workspace=self.workspace,
            embedding_func=self.embedding_func,
            meta_fields={"src_id", "tgt_id", "source_id", "content", "file_path", "keywords",
                         "description", "weight", "relation_type", "evidence_level", "source_provenance"},
        )
    ...
    async def initialize_storages(self):
        ...
        存储初始化必须逐个调用以防止死锁。此方法会:
        1. 设置默认工作空间
        2. 初始化管道状态
        3. 初始化所有存储类型（文档存储、文本块存储、实体存储等）

        Returns:
            无返回值，初始化完成后存储状态变为INITIALIZED
        """
        if self._storages_status == StoragesStatus.CREATED:
            # 设置第一个初始化的workspace为默认workspace
            # 允许命名空间操作无需为向后兼容性指定workspace
            ...
            # 为此workspace自动初始化pipeline_status
    ...
    async def get_knowledge_graph(
        ...
        获取从给定节点标签开始的知识图谱，包含节点和边。

        Args:
            node_label (str): 要获取知识图谱的节点标签
            max_depth (int): 图的最大深度，默认为3
            max_nodes (int, optional): 返回的最大节点数，默认为self.max_graph_nodes

        Returns:
            KnowledgeGraph: 包含节点和边的知识图谱对象
        """
        # 如果max_nodes为None，则使用self.max_graph_nodes作为默认值
        if max_nodes is None:
            max_nodes = self.max_graph_nodes
        else:
            # 限制max_nodes不超过self.max_graph_nodes
    ...
    def _get_storage_class(self, storage_name: str) -> Callable[..., Any]:
        ...
        根据存储名称返回对应的存储类。支持直接导入和动态导入两种方式。

        Args:
            storage_name (str): 存储名称，如"JsonKVStorage"、"NanoVectorDBStorage"等

        Returns:
            存储类构造函数

        Raises:
            ImportError: 当存储实现不存在时抛出
        """
        # 直接导入默认存储实现
    ...
    def insert(
        ...
        将文档插入RAG系统，进行文本分块、实体提取等处理。

        Args:
            input: 单个文档字符串或文档字符串列表
            split_by_character: 如果不为None，则按字符分割字符串，如果块大于chunk_token_size，将按token大小再次分割
            split_by_character_only: 如果为True，仅按字符分割，当split_by_character为None时，此参数被忽略
            ids: 文档ID或ID列表，如果未提供，将生成MD5哈希ID
            file_paths: 文件路径或路径列表，用于引用
    ...
    def query(
        ...
        Args:
            ...
            system_prompt (Optional[str]): 自定义系统提示，用于微调系统行为。默认为None，使用PROMPTS["rag_response"]
    ...
    async def aquery(
        ...
        此函数现在是aquery_llm的包装器，通过以原始格式仅返回LLM响应内容来维护向后兼容性。

        Args:
            query (str): 要执行的查询
            param (QueryParam): 查询执行的配置参数。如果提供param.model_func，将使用它而不是全局模型
            system_prompt (Optional[str]): 自定义系统提示，用于微调系统行为。默认为None，使用PROMPTS["rag_response"]
    ...
    async def adelete_by_doc_id(
        ...
        """删除文档及其所有相关数据，包括块、图元素等.

        此方法协调给定文档ID的全面删除过程。它确保不仅文档本身，
        而且跨不同存储层的所有派生和关联数据都被移除或重建。
        如果实体或关系部分受影响，将使用剩余文档的LLM缓存进行重建。

        **并发控制设计:**

        此函数实现基于管道的并发控制以防止数据损坏：

        1. **单文档删除**（当我们获取管道时）：
           - 设置job_name为"Single document deletion"（不以"deleting"开头）
           - 防止其他adelete_by_doc_id调用同时运行
           - 确保此删除的图操作独占访问

        2. **批量文档删除**（当background_delete_documents获取管道时）：
           - 设置job_name为"Deleting {N} Documents"（以"deleting"开头）
           - 允许多个adelete_by_doc_id调用加入删除队列
           - 每个调用验证job名称以确保它是删除操作的一部分

        验证逻辑`if not job_name.startswith("deleting") or "document" not in job_name`确保：
        - adelete_by_doc_id只能在管道空闲时或批量删除期间运行
        - 防止可能导致竞争条件的并发单删除
        - 当管道忙于非删除任务时拒绝操作

        Args:
            doc_id (str): 要删除的文档的唯一标识符
            delete_llm_cache (bool): 是否删除与文档关联的缓存LLM提取结果。默认为False

        Returns:
            DeletionResult: 包含删除过程结果的对象
                - `status` (str): "success", "not_found", "not_allowed", 或 "failure"
                - `doc_id` (str): 尝试删除的文档的ID
                - `message` (str): 操作结果的摘要
                - `status_code` (int): HTTP状态码（例如200, 404, 403, 500）
                - `file_path` (str | None): 已删除文档的文件路径（如果有）
        """
    ...
    async def aedit_entity(
        ...
        更新知识图谱中的实体信息，并在向量数据库中重新嵌入实体。
        还会同步entity_chunks_storage和relation_chunks_storage以跟踪块引用。

        Args:
            entity_name: 要编辑的实体名称
            updated_data: 包含更新属性的字典，例如 {"description": "新描述", "entity_type": "新类型"}
            allow_rename: 是否允许重命名实体，默认为True
            allow_merge: 当重命名到现有名称时是否合并到现有实体

        Returns:
            Dictionary: 包含更新后的实体信息的字典
        """
    ...
    def edit_entity(
        ...
        Args:
            entity_name: 要编辑的实体名称
            updated_data: 包含更新属性的字典
            allow_rename: 是否允许重命名实体
            allow_merge: 是否允许合并
    ...
    async def aedit_relation(
        ...
        更新知识图谱中的关系（边）信息，并在向量数据库中重新嵌入关系。
        还会同步relation_chunks_storage以跟踪哪些块引用了此关系。
    ...
    async def aexport_data(
        ...
        Args:
            output_path: 输出文件的路径（包括扩展名）
            file_format: 输出格式 - "csv", "excel", "md", "txt"
                - csv: 逗号分隔值文件
                - excel: 多工作表的Microsoft Excel文件
                - md: Markdown表格
                - txt: 纯文本格式输出
                - table: 将格式化的表格打印到控制台
